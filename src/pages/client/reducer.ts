...
function cacl(data: object, type) {
    Object.keys(data).forEach((key) => {
        if (['width', 'height', 'left', 'top', 'size', 'fontSize'].includes(key)) {
            if (+data[key]) {
                data[key] = CaclPixel(data[key], type);
            }
        }
    ...
    return produce(prev, (state: any) => {
        ...
        switch (type) {
            case 'reset_project_data': {
                ...
                cacl(data, data.type);
                document.body.style.fontSize = `${CaclPixel(12, type)}px`;
                return data;
            }
